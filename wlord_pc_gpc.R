## Generalised Partial Credit Model compared to Partial Credit Model
rm(list=ls())
...
#Score distributions
scssd_gpc <- sum.prob(sims,cts,cats,max.scr)
...
scssd_pc <- sum.prob(sims,cts,cats,max.scr)

plotCondSumScoreDist(Y,list(scssd_gpc,scssd_pc),list("gpcm","pcm"))
plotQQ(Y,list(scssd_gpc,scssd_pc),list("gpcm","pcm"))